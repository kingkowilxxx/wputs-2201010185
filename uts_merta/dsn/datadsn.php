...
if (isset($_POST["act"])) {
    ...
    switch ($act) {
        case "store":
                $nama = $_POST["txNAMADSN"];
                $nip = $_POST["txNIP"];
                $tgl_lahir = $_POST["txTGL"];
                $email = $_POST["txEMAIL"];
                $tlpn = $_POST["txTLPN"];
                $id_dosen = md5($nama);
                $sql = "INSERT INTO tbDSN(nama_dosen, nip, ttl ,email, notlpn , id_dosen) VALUES('$nama', '$nip', '$tgl_lahir', '$email','$tlpn', '$id_dosen');";
        ...
        case "update":
                ...
                $sql = "UPDATE tbDSN SET nama='$nama', nip='$nip', ttl='$tgl_lahir', email='$email', notlp='$tlpn'  WHERE id_dosen='$id_dosen';";
                $hasil = mysqli_query($cnn, $sql);
                if ($hasil) {
                    $footer = "<script>
                        Swal.fire({
        ...
        case "destroy":
            ...
            $sql = "DELETE FROM tbDSN WHERE id_dosen='$id_dosen';";
            $hasil = mysqli_query($cnn, $sql);
            if ($hasil) {
                $footer = "<script>
                        Swal.fire({
            ...
            <footer class="footer">
                <div class="container-fluid">
                    ...
                    <div class="copyright">
                        ...
                        <a href="javascript:void(0)" target="_blank">I Komang Widyanata Merta</a> for a better web.
                    </div>
                </div>
        </div>
    </div>
